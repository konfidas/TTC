...
public class AuthenticateSmaersAdminSystemLogMessage extends SystemLogMessage {
    ...
    public void setRoleAsBigInteger(String roleAsString) {
    ...
    @Override
        ...
        try {
            ...
            if (nextElement.getTagNo() != 1) {
                allErrors.add(new SystemLogParsingError(properties.getString("de.konfidas.ttc.messages.systemlogs.errorUserIDNotFound")));
            }
            ...
            if (nextElement.getTagNo() != 2) {
                logger.debug(properties.getString("de.konfidas.ttc.messages.systemlogs.errorUnblockResultNotFound"));
            } else {
                this.role = (DLTaggedObject) systemOperationDataIterator.next();
                this.roleAsString = DLTaggedObjectConverter.dLTaggedObjectToString(this.role);
            }

            //authenticationResult einlesen
            nextElement = (DLTaggedObject) systemOperationDataAsAsn1List.get(systemOperationDataIterator.nextIndex());
            if (nextElement.getTagNo() != 3) {
                this.allErrors.add(new SystemLogParsingError(properties.getString("de.konfidas.ttc.messages.systemlogs.errorAuthenticationResultNotFound")));
            }
            ...
            if (systemOperationDataIterator.hasNext()) {
                nextElement = (DLTaggedObject) systemOperationDataAsAsn1List.get(systemOperationDataIterator.nextIndex());
                if (nextElement.getTagNo() != 4) {
                    logger.debug("The field remainingRetries has not been found.");//NON-NLS
                } else {

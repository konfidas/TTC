...
public class AuthenticateUserSystemLogMessage extends SystemLogMessage {
    ...
    protected void parseSystemOperationDataContent(ASN1InputStream stream) throws IOException {
        ...
        try {
            ...
            if (nextElement.getTagNo() != 2) this.allErrors.add(new SystemLogParsingError(properties.getString("de.konfidas.ttc.messages.systemlogs.roleNotFound")));
            ...
            nextElement = (DLTaggedObject) systemOperationDataAsAsn1List.get(systemOperationDataIterator.nextIndex());
            if (nextElement.getTagNo() != 3) this.allErrors.add(new SystemLogParsingError(properties.getString("de.konfidas.ttc.messages.systemlogs.authenticiationResultNotFound")));
